import React, { useState, useEffect } from "react";
import { Modal } from "daisyui"; // Assuming DaisyUI's modal component is being used

const Projects = () => {
  const [projects, setProjects] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [editingProject, setEditingProject] = useState(null); // For edit mode
  const [form, setForm] = useState({
    title: "",
    description: "",
    githubLink: "",
    demoLink: "",
    image: "",
    tags: "", // Add tags field (optional)
  });
  const [isModalOpen, setModalOpen] = useState(false); // Control modal visibility

  useEffect(() => {
    const fetchProjects = async () => {
      try {
        const response = await fetch(
          `${process.env.REACT_APP_BACKEND_URL}/api/projects`
        );
        if (!response.ok) throw new Error("Failed to fetch projects");
        const data = await response.json();
        setProjects(data);
        setLoading(false);
      } catch (err) {
        console.error(err);
        setError(err.message);
        setLoading(false);
      }
    };
    fetchProjects();
  }, []);

  const handleInputChange = (e) => {
    setForm({ ...form, [e.target.name]: e.target.value });
  };

  const openModalForEdit = (project) => {
    setEditingProject(project._id);
    setForm(project);
    setModalOpen(true);
  };

  const openModalForAdd = () => {
    setEditingProject(null);
    setForm({
      title: "",
      description: "",
      githubLink: "",
      demoLink: "",
      image: "",
      tags: "", // Reset tags field
    });
    setModalOpen(true);
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    const method = editingProject ? "PUT" : "POST";
    const url = editingProject
      ? `${process.env.REACT_APP_BACKEND_URL}/api/projects/${editingProject}`
      : `${process.env.REACT_APP_BACKEND_URL}/api/projects`;

    try {
      const response = await fetch(url, {
        method,
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify(form),
      });
      if (!response.ok) throw new Error("Failed to save project");
      const savedProject = await response.json();
      if (editingProject) {
        setProjects(
          projects.map((project) =>
            project._id === savedProject._id ? savedProject : project
          )
        );
      } else {
        setProjects([...projects, savedProject]);
      }
      setModalOpen(false); // Close modal after submitting
    } catch (err) {
      console.error(err);
      setError(err.message);
    }
  };

  const handleDelete = async (id) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/projects/${id}`,
        {
          method: "DELETE",
        }
      );
      if (!response.ok) throw new Error("Failed to delete project");
      setProjects(projects.filter((project) => project._id !== id));
    } catch (err) {
      console.error(err);
      setError(err.message);
    }
  };

  return (
    <div className="p-6 bg-white shadow-lg rounded-lg">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold">Projects</h2>
        <button className="btn btn-primary" onClick={openModalForAdd}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth={1.5}
            stroke="currentColor"
            className="size-8"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="M12 9v6m3-3H9m12 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"
            />
          </svg>
          Add
        </button>
      </div>
      {error && <div className="alert alert-error mb-4">{error}</div>}
      {loading ? (
        <div className="loading loading-spinner"></div>
      ) : (
        <div>
          {/* Table displaying projects */}
          <table className="table w-full">
            <thead>
              <tr>
                <th>Title</th>
                <th>Description</th>
                <th>GitHub</th>
                <th>Demo</th>
                <th>Tags</th> {/* Add a Tags column */}
                <th>Actions</th>
              </tr>
            </thead>
            <tbody>
              {projects.length > 0 ? (
                projects.map((project) => (
                  <tr key={project._id}>
                    <td>{project.title}</td>
                    <td>{project.description}</td>
                    <td>
                      <a
                        href={project.githubLink}
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        View Code
                      </a>
                    </td>
                    <td>
                      <a
                        href={project.demoLink}
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        View Demo
                      </a>
                    </td>
                    <td>{project.tags.split(",").map(tagItem=> <span className="inline-block bg-blue-100 text-blue-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded-full" key={tagItem} 
                    >{tagItem}</span>) || "No tags"}</td> {/* Show tags or "No tags" */}
                    <td>
                      <button
                        className="btn btn-sm btn-warning mr-2"
                        onClick={() => openModalForEdit(project)}
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          fill="none"
                          viewBox="0 0 24 24"
                          strokeWidth={1.5}
                          stroke="currentColor"
                          className="size-4"
                        >
                          <path
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10"
                          />
                        </svg>
                      </button>
                      <button
                        className="btn btn-sm btn-error"
                        onClick={() => handleDelete(project._id)}
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          fill="none"
                          viewBox="0 0 24 24"
                          strokeWidth={1.5}
                          stroke="currentColor"
                          className="size-4"
                        >
                          <path
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0"
                          />
                        </svg>
                      </button>
                    </td>
                  </tr>
                ))
              ) : (
                <tr>
                  <td colSpan="6">No projects found</td>
                </tr>
              )}
            </tbody>
          </table>
        </div>
      )}

      {/* Modal for Adding/Editing Projects */}
      {isModalOpen && (
        <div className="modal modal-open">
          <div className="modal-box">
            <h3 className="text-lg font-bold mb-4">
              {editingProject ? "Edit Project" : "Add Project"}
            </h3>
            <form onSubmit={handleSubmit}>
              <input
                type="text"
                name="title"
                value={form.title}
                onChange={handleInputChange}
                placeholder="Project Title"
                className="input input-bordered mb-2 w-full"
                required
              />
              <textarea
                name="description"
                value={form.description}
                onChange={handleInputChange}
                placeholder="Project Description"
                className="textarea textarea-bordered mb-2 w-full"
                required
              ></textarea>
              <input
                type="text"
                name="githubLink"
                value={form.githubLink}
                onChange={handleInputChange}
                placeholder="GitHub Link"
                className="input input-bordered mb-2 w-full"
              />
              <input
                type="text"
                name="demoLink"
                value={form.demoLink}
                onChange={handleInputChange}
                placeholder="Demo Link"
                className="input input-bordered mb-2 w-full"
              />
              <input
                type="text"
                name="image"
                value={form.image}
                onChange={handleInputChange}
                placeholder="Image URL"
                className="input input-bordered mb-2 w-full"
                required
              />
              <input
                type="text"
                name="tags"
                value={form.tags}
                onChange={handleInputChange}
                placeholder="Tags (comma separated)"
                className="input input-bordered mb-2 w-full"
              />

              <div className="modal-action">
                <button className="btn" type="submit">
                  Save
                </button>
                <button
                  className="btn btn-outline"
                  type="button"
                  onClick={() => setModalOpen(false)}
                >
                  Cancel
                </button>
              </div>
            </form>
          </div>
        </div>
      )}
    </div>
  );
};

export default Projects;
